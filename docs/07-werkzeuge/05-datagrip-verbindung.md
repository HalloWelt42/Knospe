...
Die Postgres-Datenbank läuft im Container. Nach außen ist sie über einen Host-Port erreichbar. Dieser Port ist nicht fest: Das Skript [`tools/setup-env.sh`](../../tools/setup-env.sh) leitet ihn pfad-eindeutig ab (per `cksum` in die [`.env`](../../.env)), damit sich zwei Installationen von Knospe nie in die Quere kommen.
...
4. Fülle "User", "Password" und "Database" mit den Werten aus der [`.env`](../../.env) (`POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`).
