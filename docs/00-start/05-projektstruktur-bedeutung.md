...
Die Klasse `Knospe\Domain\Post\PostService` liegt also zwingend in [`backend/src/Domain/Post/PostService.php`](../../backend/src/Domain/Post/PostService.php). Diese Symmetrie ist kein Zufall: Sie erlaubt es, jede Klasse allein am Namen im Dateibaum zu finden, ohne zu suchen. Composer lädt sie dann automatisch, sobald sie gebraucht wird.
...
2. Einen Eintrag in [`backend/config/routes.php`](../../backend/config/routes.php), der die Adresse `/api/posts/latest` mit deinem Handler verbindet. Der Router aus `backend/src/Core/` liest diese Tabelle und findet zur Anfrage den passenden Handler.
...
Das Frontend spiegelt diese Ordnung. Datenmodelle wie ein Beitrag stehen im Backend als readonly-Entity in `backend/src/Domain/Post/`, im Frontend als TypeScript-Typ in [`frontend/src/lib/types.ts`](../../frontend/src/lib/types.ts). Der API-Client [`frontend/src/lib/api.ts`](../../frontend/src/lib/api.ts) ruft genau die Endpunkte auf, die das Backend anbietet. So findest du zu jeder Backend-Route die passende Frontend-Stelle - und umgekehrt.
