...
- **PSR-4 (Autoloading):** Ein Namespace-Präfix wird auf einen Ordner abgebildet. Bei uns `Knospe\` auf `backend/src`. Die Klasse `Knospe\Domain\Post\PostService` liegt also in `backend/src/Domain/Post/PostService.php`. Composer lädt sie automatisch, kein manuelles `require` nötig.
- **PSR-7 (HTTP-Nachrichten):** Anfrage und Antwort sind Objekte mit klaren Methoden, nicht die rohen PHP-Globals. In `backend/public/index.php` wird die Anfrage per `fromGlobals()` in ein PSR-7-Objekt umgewandelt.
- **PSR-11 (Container):** Ein einheitlicher Vertrag mit `get()` und `has()`, um Dienste zu holen. Unser `backend/src/Core/Container.php` implementiert `Psr\Container\ContainerInterface`.
